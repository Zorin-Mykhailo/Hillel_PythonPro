...
class UsrInput:
    ...
    @staticmethod
    def get_str(input_str: str, min_len: int = 0, str_constraints: list[str] = []) -> str:
        ...
        while _user_input is None:
            ...
            if min_len is not None and _user_input is not None and len(_user_input) < min_len:
                print(f"\33[41mПОМИЛКА ВВОДУ ДАННИХ.\33[0m Ведіть рядок не коротше ніж {min_len} символи")
                _user_input = None
            if str_constraints is not None and len(str_constraints) > 0 and _user_input not in str_constraints:
                print(f"\33[41mПОМИЛКА ВВОДУ ДАННИХ.\33[0m Ведіть рядок з поміж доступних варіантів ({str_constraints})")
                _user_input = None
        return _user_input
